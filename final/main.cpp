...
public:
    ...
    void run()
    {
        ...
        address = SocketAddress("192.168.223.169", 8000);
    ...
    bool send(){
        ...
        while (1)
         {  
            ...
            result = _socket.recv(buffer, remaining_bytes);
            printf("buffer: %s\n", buffer);
            if(buffer[0] != 'n'){
                if(buffer[1] != 'n'){
                    int light = (buffer[1] - '0') * 10 + (buffer[2] - '0');
                    float dutycycle = (float(light)*100+float(light)) / 10000.0;
                    led.write(dutycycle);
                    printf("haihai\n");
                }
